...
const int buzzerPin = 6; // Substitua 7 pelo pino que você conectou o buzzer
...
void updateCountdown() {
  if (countdownValue > 0) {
    ...
    // Tons da sirene
    int sireneTones[] = {1000, 1500, 2000, 2500}; // Frequências da sirene

    // Reproduza a sirene
    for (int i = 0; i < 4; i++) {
      tone(buzzerPin, sireneTones[i]);
      delay(500); // Duração de cada tom
      noTone(buzzerPin);
      delay(200); // Pausa entre os tons
    }
  }
}
